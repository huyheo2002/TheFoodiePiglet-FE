...
function Button({ to, href, children, onClick, iconLeft, iconRight, variant, disabled = false }) {
  let Comp = "button";
  const props = {
    onClick,
  };
  ...
  const classes = clsx(
    "px-3 py-2 rounded-lg border-2 border-solid border-transparent my-1 mx-2 cursor-pointer inline-flex justify-center items-center transition-all duration-300 select-none whitespace-nowrap text-ellipsis overflow-hidden",
    {
      ...
      "bg-[#1E5631] shadow-black-b-0.75 hover:-translate-y-1 hover:bg-[#16794E] hover:shadow-black-b-0.35":
        variant === "excel",
      "!cursor-not-allowed bg-[#b3b3b3] shadow-black-b-0.75 hover:-translate-y-1 hover:bg-[#414141] hover:shadow-black-b-0.35": disabled
    }
  );

  const classesChild = clsx("text-sm font-normal tracking-wider text-white", {
    "text-white": variant === "primary",
    "text-white font-bold text-sm": variant === "viewMore" | "baseOrange" | "delete" | "success" | "excel",
  });
  ...
  return (
    <Comp className={classes} {...props}>
      ...
      {children && (
        <span className={clsx("px-1 text-white", classesChild)}>
          {children}
        </span>
      )}

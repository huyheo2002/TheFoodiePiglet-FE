...
function InputCheckbox({ ...props }) {
  ...
  return (
    <div
      className={clsx("custom-radio-container", {
        "!hidden": hidden,
      })}
    >
          ...
          group.map((itemGroup) => {
            return (
              <Fragment key={itemGroup.id}>
                <div className="text-sm text-black font-semibold my-1">
                  {itemGroup.name}
                </div>
                {options.map((option, index) => {
                  let cloneId = Math.floor(Math.random() * 9999);
                  let itemChecked = false;
                  let filterItemChecked =
                    Array.isArray(listItemSelected) &&
                    listItemSelected.length > 0 &&
                    listItemSelected.filter((itemSelected) => {
                  ...
                  if (itemGroup.id === option.idGroup) {
                    return (
                      <div
                        className={clsx("flex items-center py-1 ml-2", {
                          "inline-flex": row,
                        })}
                        key={index}
                      >
                        ...
                        />
                        <label
                          className="capitalize text-sm font-medium text-black tracking-wider pr-6"
                          htmlFor={cloneId}
                        >
                          {option.label}
                        </label>

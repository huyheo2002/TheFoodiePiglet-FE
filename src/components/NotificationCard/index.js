...
function NotificationCard({ data, page, onClick }) {
  ...
  const filterUser =
    (listUser.length > 0 &&
      data &&
      listUser.filter((item) => item.id === data.userId)) ||
    [];
  ...
  return (
    <div
      className={clsx(
        "flex select-none px-2 py-3 rounded-md hover:bg-[#e6f2fe]",
        {
    ...
    >
      ...
      <div
        className={clsx("w-[calc(100%-48px)] pl-3", {
          "w-full px-3": page === "user",
        })}
      >
        <p
          className={clsx(
            "text-black font-light text-sm line-clamp-3 leading-4",
            {
        ...
        >
          <span
            className={clsx(
              "inline-block font-semibold text-base text-black capitalize pr-1 leading-4",
              {
        ...
        <span
          className={clsx("font-medium text-sm text-gray-700 mt-1 block", {

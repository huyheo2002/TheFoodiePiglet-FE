            ...
            <li className="h-full leading-[4rem] relative group" key={index}>
              {item.href && (
                <Fragment>
                  ...
                  item.keyword === "login" ? (
                    <a
                      className="h-full px-3 inline-flex items-center text-black font-semibold uppercase text-sm group-hover:text-primary transition-all duration-300"
                    >
                      {!item.onlyShowIcon ? (
                        <Fragment>
              ...
              {item.to && (
                <Fragment>
                  ...
                  userLogin.auth === true &&
                  item.keyword === "login" ? (
                    <Link className="h-full px-3 inline-flex items-center text-black font-semibold uppercase text-sm group-hover:text-primary transition-all duration-300">
                      {!item.onlyShowIcon ? (
                        <Fragment>
                          {t(`userLogin.msgWelcome`) + `, ${userLogin.username}`}
                          {item.icon && (
              ...
              {!item.href && !item.to && (
                <Fragment>
                  <div className="h-full px-3 inline-flex items-center text-black font-semibold uppercase text-sm group-hover:text-primary transition-all duration-300">
                    {!item.onlyShowIcon ? (
                      <Fragment>
                        {item.keyword && t(`nav-item.${item.keyword}`)}
                        {item.icon && (

...
function Cart() {
  ...
  const handleOnSubmitBooking = async (e) => {
    ...
    const totalPriceNumber = dataEntry.totalPrice
      ? parseFloat(dataEntry.totalPrice.replace(/\$/g, ""))
      : 0;
    ...
    try {
      ...
      if (respon && respon.errCode === 0) {
        ...
        await cartServices
          .handleRefreshCart(dataUser.user.id)
          .then(() => setListItemInCart([]));
  ...
  return (
    <div className="flex flex-col items-center justify-center container overflow-hidden mx-auto">
        ...
        {listItemInCart.length > 0 ? (
          listItemInCart.map((item, index) => {
            return (
              <ItemCompact
                key={index}
                size={"oneItems-onRows"}
                type={"cart"}
                data={item}
                onHandleRefreshCart={handleReloadItemInCart}
              />
            );
          })
        ) : (
      ...
      {/* order success */}
      <Modal
        open={openModalOrdersSuccess}
        custom
        close={handleCloseModalOrdersSuccess}
      >
